...
func (TssCommonTestSuite) TestTssCommon(c *C) {
	...
	tssCommon := NewTssCommon(peerID.String(), broadcastChannel, TssConfig{}, "message-id", 1)
	...
	c.Assert(err, IsNil)
	wrapMsg := fabricateTssMsg(c, btss.NewPartyID("1,", "test", bi), "roundInfo", []byte("message"))
