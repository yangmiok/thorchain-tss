...
func (t *TssServer) KeySign(req keysign.KeySignReq) (keysign.KeySignResp, error) {
	...
	msgID,committeeID, err := t.requestToMsgId(req)
	if err != nil {
	...
	keySignMsgChannel, keySignSyncChannel, signatureChannel := keysignInstance.GetTssKeySignChannels()
	t.p2pCommunication.SetSubscribe(p2p.TSSKeySignMsg, msgID, keySignMsgChannel)
	t.p2pCommunication.SetSubscribe(p2p.TSSKeySignVerMsg, msgID, keySignMsgChannel)
	t.p2pCommunication.SetSubscribe(p2p.TSSKeySignSync, msgID, keySignSyncChannel)
	t.p2pCommunication.SetSubscribe(p2p.TSSSignature, committeeID, signatureChannel)
	...
	defer t.p2pCommunication.CancelSubscribe(p2p.TSSSignature, committeeID)

	signatureData,signedMsg, err := keysignInstance.SignMessage(req)
